...
- Windows OCR 与批注（v0.4 收尾，§8.10/§8.8）：
  - 图片 OCR（Windows.Media.Ocr 系统能力，专用 MTA 线程）：图片查看器一键识别，
    识别文本写入提取缓存与 FTS——图片文字可被全文搜索命中；
    未装语言包时给出安装指引；当前 SDK 元数据未提供置信度字段（如实标注）；
  - 图片查看器：双击图片预览、缩放适配、系统打开；
  - 批注（Markdown/文本）：引用编辑器选中文本添加批注、列表、解决/重开/删除；
    内容变化后引用无法定位时标注「位置已变」，不静默丢失；
  - 编辑器侧栏 Tab 化：AI 助手 / 批注 面板并存切换，选区捕获适配
    Tiptap 与 CodeMirror 两种编辑器。
